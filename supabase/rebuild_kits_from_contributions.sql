-- ============================================================
-- Kits & member balances = contributions only (not loan activity)
-- Loans track exposure in loan_books; available to lend = deposits − outstanding
-- ============================================================

-- Rebuild all kit pots and personal balances from completed contributions
...
begin
  ...
  loop
    -- Only chair/treasurer when scoped to one chama
    ...
    -- Zero kits then fill from contributions
    ...
    -- Member balances from their contributions only
...
-- Available liquidity = contribution-backed kits minus principal still out
...
as $$
  select greatest(
    0,
    coalesce((
      select sum(k.balance)
      from public.chama_kits k
      where k.chama_id = p_chama_id
        and k.kit_code = any (public.loan_liquidity_codes())
    ), 0)
    - coalesce((
      select sum(lb.principal_remaining)
      from public.loan_books lb
      where lb.chama_id = p_chama_id
        and lb.status = 'active'
    ), 0)
  );
$$;

-- Disburse: freeze mix from contribution balances; do NOT reduce kit pots
...
begin
  ...
  if v_pool < p_amount then
    raise exception 'Available loaning pool has only %, cannot disburse %', v_pool, p_amount;
  end if;
  ...
  -- Freeze allocation % from contribution-backed kit balances (no debit)
  ...
  if v_remaining > 0.01 then
    v_allocations := v_allocations || jsonb_build_object(
      'member-loans',
      coalesce((v_allocations->>'member-loans')::numeric, 0) + v_remaining
    );
    v_remaining := 0;
  end if;

  -- Kits unchanged (still = contributions). Only loan_books track money out.
...
-- Repay: principal reduces loan_books only (kits stay = deposits).
-- Interest still funds group-reserve + member share credits.
...
begin
  ...
  -- Principal: only reduces outstanding (kits remain contribution totals)
  -- Available pool rises automatically via loaning_pool_balance()
  ...
  return jsonb_build_object(
    ...
    'loaning_pool_available', public.loaning_pool_balance(p_chama_id),
    'reference', v_ref
  );
end;
$$;
...
-- One-shot: clear loan books and rebuild kits from deposits (optional clean slate)
-- Uncomment and run if you want zero open loans + pure contribution balances:
--
-- truncate public.loan_books;
-- update public.chama_members set active_loans = 0;
